...
Deploys to Vercel with Neon Postgres and Cloudflare R2. `vercel.json` sets the build command to `pnpm ci` and schedules a cron every 10 minutes against `/api/payload-jobs/run`.

`vercel.json` also sets `ignoreCommand` to `scripts/vercel-ignore-build.sh`, which cancels the build when every file changed since the last successful deployment on the branch is one the site never reads (docs, agent and editor config, tests, stories, lint and Storybook tooling). It fails open: an unlisted path, a missing base commit, or a same-commit redeploy all build. A skipped push shows as **Canceled** in Vercel, which is expected. To force a build anyway, redeploy from the dashboard with **Use project's Ignore Build Step** unchecked. New inert paths go in `INERT_RES` in that script.

The Payload job queue drives newsletter sends and scheduled publishing, and it drains only when that endpoint is hit, so the Vercel cron (Pro plan, sub-daily schedules) is the scheduler. `.github/workflows/payload-jobs.yml` declares the same 10-minute cadence but GitHub drops most scheduled runs on shared runners (about 8 a day in practice), so it is only a best-effort backstop. It needs the `PAYLOAD_JOBS_URL` and `CRON_SECRET` repo secrets. `VERCEL_AUTOMATION_BYPASS_SECRET` is optional: it matters only if production sits behind Vercel Deployment Protection, which it does not.

Required Vercel env vars: `PAYLOAD_SECRET`, `CRON_SECRET`, `PREVIEW_SECRET`, Resend keys, and the `R2_*` / `NEXT_PUBLIC_MEDIA_URL` media vars. `POSTGRES_URL` is set by the Neon integration. `PRODUCTION_DB_ENDPOINT` (the production Neon endpoint id, `ep-...`) feeds `scripts/guard-preview-db.ts`, which `pnpm ci` runs before `payload migrate`: preview deploys get a Neon branch (`preview/<git-branch>`) from the Neon deployment action on the store connection, and the guard aborts any non-production build that still sees the production endpoint. Git pushes build `main` only (`vercel.json` `git.deploymentEnabled`); a `preview` canary is created on demand from the Vercel dashboard (Deployments, Create Deployment, branch `preview`) so each merge costs one build, not two.
