...
| Takeover menu (GSAP + VT) | The page frame docks into the menu's preview slot (scale + trailing clip mask); across the dock's back half the window cross-fade dissolves into the current page's hero media — cloned from the hero's `data-hero-media` region (every hero in `src/heros/*` marks one) into a layer injected inside the frame, so the animating mask crops the fade; media-less pages keep the scaled page view; hovering a menu link cross-dissolves the window (fast ease-out) to that page's hero media — sourced server-side per link by `getMenuContent`, stacked inside the same layer so the mask still crops it, dissolving back to the resting state when the pointer leaves; the footer bar fades out immediately (page chrome, never part of the dock); menu content staggers in from the dock's halfway point; the Ask transcript swaps in over the window via a React View Transition (`menu-chat` recipe) | Motion consts at the top of `src/Header/Menu/index.tsx`; the swap recipe in `view-transition.css` | Storybook — `Features/TakeoverMenu` |
