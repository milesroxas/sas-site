...
- Never hand-roll a new reveal tween. If a shell needs different choreography (see `WorkIntro/Section.client.tsx`), it still imports the shared gate (`SCROLL_REVEAL_TRIGGER_DEFAULTS`) and any value one of the reveals already owns (ease, durations, the intro stagger) instead of restating it.
