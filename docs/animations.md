...
- One timeline and one viewport gate per shell; the whole entrance reverses out when the section leaves, so each pass replays. Full-viewport shells (`fullViewportSectionClassName` surfaces) raise the enter gate to `SCROLL_REVEAL_FULLSCREEN_ENTER_THRESHOLD` so the entrance lands while the band owns the screen; shorter shells keep `SCROLL_REVEAL_TRIGGER_DEFAULTS`. Reduced motion renders the final state; server-rendered children stay visible without JS.
