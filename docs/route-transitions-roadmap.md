...
| Piece | File | Role |
|---|---|---|
| Page wrapper | [src/app/(frontend)/template.tsx](../src/app/(frontend)/template.tsx) → [DirectionalTransition.tsx](../src/shared/ui/view-transition/DirectionalTransition.tsx) | Keyed on the pathname (a template alone only remounts when its child segment changes); maps transition types → CSS classes; `default: 'none'` everywhere |
...
- [ ] Stage 3 — edge-case hardening (D3 open) *(2026-09-10: same-segment navigations, `/works/a` → `/works/b`, now transition; see log)*
...
- **2026-09-01** — **Menu navigations suppress the platform view transition** (user report: the handoff traveler snaps mid-expansion, after the menu has exited, while the media expands toward full screen — the third report on this path). Root cause, at last: *untagged is not un-transitioned*. React flags a view transition in its before-mutation pass for any newly placed subtree that contains a `<ViewTransition>` (`trackEnterViewTransitions`), and `(frontend)/template.tsx` remounts one on every route change — before any class is resolved. So `default: 'none'` silences the animation but `document.startViewTransition()` still runs on every navigation, including the handoff push. A started transition suspends the document's rendering, and with it `requestAnimationFrame`, from the capture until React's update callback resolves — and React deliberately holds that callback for up to 500ms waiting on the incoming page's above-the-fold images and fonts (`SUSPENSEY_FONT_AND_IMAGE_TIMEOUT`). The GSAP traveler therefore stalls for that window at route commit; GSAP's ticker smooths gaps only *longer* than 500ms, so the next tick jumps the expansion straight to its time-correct progress. Every previous fix on this path (share type maps, `data-menu-handoff`) addressed what the transition *paints*, never that it runs at all — which is why the snap survived them. Fix: `suppressViewTransitions()` (`shared/lib/view-transition/suppress.ts`) removes `document.startViewTransition` for the flight, putting React on its own no-transition commit path — the same one browsers without view transitions take (§3 already accepts it). Held by both menu paths: the hero handoff for the traveler's whole flight, the plain undock close from the click until the reverse completes. The CSS guards stay as belt. **Rule going forward: a navigation whose motion is owned imperatively must suppress the platform transition, not just leave it untagged.**
- **2026-09-09** — **Plain-close menu navigation holds its media and masks the page in** (user report, with a screen recording: leaving a page whose docked window shows fallback media, the media fades out, the old page shows, then the new page snaps in and flickers). Root cause: the plain undock close reverses the whole open timeline, including the base dissolve, so the window's media faded OUT over the old page's crop while the route committed beneath and cut to the new page mid-undock, with the new page's entrances playing inside the still-scaling frame: a fade between two different pages followed by a hard cut, both rejected by §4. Fix: `Menu/navCurtain.ts`. The click takes the window's opaque media stack out of the timeline and pins it; the undock expands it to full screen with the frame (the handoff traveler's move, without a traveler); at the reverse's end the stack moves into a fixed curtain in the same task the frame is restored; once the route has committed and painted the curtain lifts with the default lateral reveal (`vt-curtain-lift`, `--vt-duration-reveal` / `--vt-ease-reveal`), so a menu navigation lands on the page exactly like a lateral link does. Platform transition suppressed for the flight (same rule as the handoff); every gate has a timeout. Reduced motion, a mid-dissolve window, a cold base, and same-pathname clicks keep the plain undock. The hero handoff path is untouched.
- **2026-09-10**: **Same-segment navigations get the transition; the roll's unmount no longer writes scroll** (user report: on mobile, the featured-work roll at the foot of a case study into another case study breaks the transition and does not land at the top). Traced with an instrumented mobile Playwright run against production. Root cause of the broken transition: `(frontend)/template.tsx` remounts only when the segment directly below it changes (`works` → `insights`), so `/works/a` → `/works/b` kept `DirectionalTransition` mounted, React placed no `<ViewTransition>`, `document.startViewTransition()` was never called, and the navigation was a hard cut on every device (the header logo from the same page animated fine). `DirectionalTransition` now keys the boundary on `usePathname()`; search-param and hash changes keep the pathname and stay silent. Second finding from the same trace: 7ms after `pushState`, and before `LenisRouteReset`, the page scrolled to where the roll's last snap glide had started. `useGSAP`'s `context.revert()` rewinds every tween it recorded, GSAP re-enters a tween's context inside its callbacks so ScrollTrigger's snap tween (created from its delayed call) joins the roll's context, and reverting it wrote the glide's start value through the Lenis scroller proxy. The route reset overwrote it in the same commit, so it was latent for navigation, but it is a stale write on the one position Lenis owns, and it would have moved the page on a `revertOnUpdate` remount with no reset following. Rule: a scroll tween is killed, never reverted; the roll's scroll-driven timeline now lives outside its `useGSAP` context, in an effect of its own that kills it on teardown. Not reproduced in emulation: any final scroll position other than 0 (Chromium and WebKit, iPhone emulation, dev and production, tap at rest, mid-glide and mid-inertia all landed at 0); re-check on device after this ships.
- *(pending)* D3 pending affordance · reveal duration/ease final values (dial on `/demo/transitions`).
- **2026-09-09**: **Simulator brought back to parity with the site** (user report: `/demo/transitions` had drifted and no longer followed the standard). The route simulator had three of the nine `:root` variables wired (so Copy would have dropped six), no `work-open` path at all, a post-image morph whose mock ends used different crops (the very drift `POST_IMAGE_FRAME` exists to prevent), and ease options the standard rejects on an entrance (an overshooting `back` curve on a clip-path inset, an in-out curve on an entrance). Now: every variable is live and Copy emits the whole `:root` block in order; the panel seeds from the live stylesheet on mount instead of restating numbers; the mock site has one surface per shipped navigation (insight cards in both index variants and a `PostHero` at `POST_IMAGE_FRAME`, a `/works` index of rows, an Industry work spotlight that opens its case study by the real `work-open` takeover through `sequenceWorkImageMorph` and `WorkImageTransition`, both case-study hero layouts so an axis already home visibly drops out of the landing, back links, pagination, the untagged browser back); the timeline plots the takeover's chained beats and its measured landing; the window fills the viewport for a takeover, because the sequencer measures `window.inner*`. Finding 7 closed. The menu's two GSAP navigations stay out of the simulator by design (they suppress the platform transition); the overview names them and where each half is tuned.
