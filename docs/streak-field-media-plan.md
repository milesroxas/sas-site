...
- Code owns particle counts, DPR, segmentation, noise complexity, simulation, backend, and transitions. Never accept raw shader code, arbitrary props/import paths, or `force` from CMS data. (2026-09-18: count moved into Studio's authoring contract, bounded by the placement ceilings, because density cannot otherwise be authored: at a fixed count, widening the pitch only packs the surviving rows tighter. DPR, segments and octaves stay code-owned, and no CMS field exposes any of them.)
...
Resolve once: defaults → named preset deltas → validated overrides → placement/device ceilings. Keep tuning in [presets.ts](../src/features/immersive/presets.ts), defaults beside the effect, and external effect imports through `@/features/immersive`. A small dependency-free ID/label/poster contract is appropriate; a second tuning provider would conflict with [immersive-effects.md](immersive-effects.md). Since the light leak joined the Streak Field as a slot visual, that contract is `EffectContract` ([studio-effects.md](studio-effects.md)): closed, dependency-free, and it points at each effect's defaults and presets without holding a tuning.
...
- Posters: `public/images/streak-field/<look>-<surface>.webp` with alpha, rendered by `scripts/visual-posters.ts` from the `Immersive/VisualPosters` capture rig; both ground polarities ship and globals.css picks one by the nearest `[data-theme]` or `.band-dark`.
...
- Decision: **bounded local delivery**, recorded above. Unsupported live placements: menu hover previews and the handoff traveler, cards, repeated and pinned work previews, carousels and galleries (all posters by `PLACEMENT_LIMITS`). The one live menu surface is the docked window at rest, below. Touch-first devices, reduced motion, software renderers and contexts without a renderable float target (for `flow` looks) stay on posters.
...
- Live docked window (2026-09-14): a page whose hero is a Streak Field used to rest in the menu on its still, which on a dark band reads as an empty black box, while a video hero keeps playing in the window. The window now runs the page's own field. `StreakVisual` publishes its resolved descriptor on the slot root (`data-visual-descriptor`, `serializeStreakDescriptor` / `parseStreakDescriptor`, the poster upload reduced to what `mediaPosterImage` reads); `mountHeroMedia` reads it off the cloned hero (`readHeroStreakSource`: descriptor, band ground, poster `sizes`) and leaves a host in the dissolve layer above the base clone, under the hover previews and the Ask cover. `TakeoverMenu` portals `MenuLiveVisual` (`src/Header/Menu/LiveVisual.tsx`) into it: a `StreakVisual` at placement `menu` on the band's ground, its own poster the same rendition the clone holds, so mounting changes no pixel and the field reveals with the slot's crossfade. The host is sized to the window's crop in screen pixels and counter-scaled by the dock's inverse (`fitToWindow` with `counterScale`, tracked every tick of the open timeline by `trackWindowScale`), since the field composes in CSS px and would otherwise be a sub-pixel copy of the hero; it is revealed on the open's `onComplete` and faded on the base's beat at close. `active` is the menu's verdict (`liveActive`): open and settled, no hover preview over it, no Ask transcript; a close, a hover or Ask stop frames at once. `usePageCovered` is off for the `menu` placement (the window is what covers the page); the hero's own slot releases the lease while covered, so the ceiling of one holds. `PLACEMENT_LIMITS.menu` is `live` at DPR 2, count 1000, flow allowed. Reduced motion, coarse pointers and software renderers keep the still in the window as everywhere else. Hover previews and the handoff traveler remain posters.
...
- Tabs blocks (2026-09-16): the same `visualSlotFields` wraps the `media` upload inside each tab row of `featureTabs` (optional upload) and `audienceTabs` (required-unless-shader). The blocks' `Component.tsx` files are now server adapters, like Stacked's: `resolveRowVisuals` (`src/blocks/shared/row-visuals.ts`) runs `resolveVisual` per row, seeded from the row id, and hands the client strip (`FeatureTabs.tsx`, `AudienceTabs.tsx`) a `Visual` per tab with the stored slot fields removed, so a retained upload never crosses to the client. Both render through the `Visual` adapter at placement `block` on `blockSurface(theme, bare)`; Radix mounts only the active Tabs panel and the Audience tabs swap keys the incoming slot on the tab id, so at most one tab's field is live per block. The Audience tabs outgoing underlay renders at placement `card` (poster only) so the wipe never contends for the lease. `collectVisualMediaRefs` now walks `tabs[]` rows (media and shader poster) like carousel slides, closing a gap where tab media was never publish-gated. Work Pages keep scoping the tab pickers and posters through `withCaseStudyScopedMedia`, which already descends arrays and groups.
...
| Tabs and Audience tabs, per tab (top level or, for Tabs, inside a Section) | Yes (placement `block`) | Yes | Each tab row is its own slot; only the active tab's field is live. The Audience tabs swap underlay is placement `card`, poster only. |
| Takeover menu docked window, at rest on a page whose hero is a Streak Field | Yes (placement `menu`) | Yes | The page's own field, read off its hero; 1:1 screen pixels; stops under a hover preview, the Ask transcript and the close. |
| Takeover menu hover previews and handoff traveler | No | Yes | Posters only; `hero` eligibility follows the destination's own visual. |
...
Migration `20260914_032613_streak_field_visuals` (regenerated 2026-09-14 after `menuPreviewType` gained `defaultValue: 'automatic'`) carries the original rollout. The tabs slice (2026-09-16) has its own additive migration, `20260916_161637_tabs_visual_slots` (generated with no create-or-rename prompts): `visual_type` and the `shader_*` columns on every `*_aud_tabs_tabs` and `*_blocks_feature_tabs_tabs` table (plus `_v` copies) under Pages, Home, Posts, the segment pages, Lab Pages and Work Pages. The answer sheet below covers both; regenerate if the branch is rebased past a newer snapshot.
...
Run when regenerating (after rebasing on `main` and with approval): `pnpm migrate:create streak-field-visuals`. For the tabs slice on its own: `pnpm migrate:create tabs-visual-slots`.
