...
| Rich text (`richText`) | `rich-text/Component.tsx` | new block (2026-09-03) | body cols 3-6; prose mode, `text-base` stepping to `text-lg` from `xl`. Insights added from the editor (2026-09-04) split the body into cells: each prose run cols 3-6, an insight run is a subgrid at col 3 spanning 4 (one insight fills it, two share it) or 6 (three or more, three per row, out to col 8) | DONE 2026-09-03 |
