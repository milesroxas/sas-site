...
**Status 2026-09-21.** The judge is `on` everywhere: production (step 9, moved from `shadow` by Miles), preview, and the main checkout's local `.env`. The journey's page search and prompt lines, and the thin case study path (feature README, "A thin case study", built the same day), now act on production turns instead of being measured. Done the same day: step 2 (the key and `ASK_JEV=on` are in `~/conductor/repos/sas-site/.env`, so new workspaces match; existing workspaces keep the `.env` they were created with) and step 10 (see [Analytics: what is done and what is owed](#analytics-what-is-done-and-what-is-owed)). Still open: step 6 (the human read of `minimal` answers) and the manual script run once on the live site (step 9's done-when). Production never got its week of `shadow`: watch `judge_failed`, `fell_back` and `first_output_ms` on `ask_questioned` for the first weeks instead, and read [In production: reading shadow](#in-production-reading-shadow) as the list of what would send it back.

**Status 2026-09-19, evening** (kept for the record). Done: step 1 (key in Vercel, Production and Preview), step 4 (preview `on`, confirmed by card-only turns in 300 to 565 ms), step 7 (the Privacy Policy, Pages id 8, was already published; it now names TypeSafe in sections 6 and 7, describes the journey, and carries the new effective date, on production and preview), step 8 (production `shadow`, confirmed in the log: `judge_mode: "shadow"`, `judge_failed: false`, `judge_ms: 387`, `page_leaned: true`). Skipped: step 3, the `preview-off` baseline, since preview went `on` first; to get one, set preview `off`, redeploy, capture, and set it back. Open: steps 2, 5, 6, 9 and 10. Production stays `shadow` until Miles moves it: the journey's page search and prompt lines act only in `on`, so on production today they are measured (`page_leaned`), not applied. The single shared conversation is live everywhere.


Written for Miles. In order: each step unblocks the next, and nothing here needs a code change. The code is on `main`, `preview` and `dev` (`d7d399c`). When this table was written the judge was `off` everywhere it was deployed; as of 2026-09-21 steps 1, 4, 7, 8 and 9 are done and it is `on` everywhere (see the status above).
...
| 13 | New chat, on `/works/gentlebeast` (a case study whose story is under 200 words, see the feature README, "A thin case study"): "What did you do for them?" | An answer that names the kinds of work (product design, web design, UX strategy, brand identity) from the record's brief, with `/works/gentlebeast` first among the sources, **then** the `case_study` card: "The full case study is still being written. A partner can walk you through it and similar work." The words must not say the case study is unfinished: the card does. The same question on `/works/interchecks` (a written story) gets a plain answer and the quiet offer |
| 14 | Open any card's form and send it (use a real inbox you own). Then ask "Can I talk to someone?" | A reply in words and **no second card**: once the visitor has sent, never a card |
| 15 | Ask a long question (number 6 works) and press Stop mid-answer | The reply stops, no card is appended, and the row in the admin says `stopped` |
...
| 2 | TypeSafe becomes a processor of visitor questions. It does not train on requests; zero retention is enterprise only. The drafted Privacy page (Pages id 8, unpublished) should list it | Add before Jev sees production traffic. **Done 2026-09-19**: the published Privacy Policy names TypeSafe in sections 6 and 7, and production has run the judge since | Phase 2 in production |
...
**Done 2026-09-21**, over the PostHog MCP:

- **Schema.** `read-data-schema` lists fifteen of the judge's properties on `ask_questioned`: `judge_mode`, `judge_ms`, `judge_failed`, `judge_request`, `judge_confidence`, `chunks_candidates`, `chunks_kept`, `passages_ms`, `first_output_ms`, `model_skipped`, `fell_back`, `answer_model`, `journey_pages`, `page_leaned`, `page_attached`. Two are absent, both as expected: `judge_agrees` has never carried a value (PostHog registers a property on its first non-null value, no captured turn ever ran in `shadow`, and production is now `on`), and `story_thin` ships with the thin case study code.
- **Tiles.** Nine on the **Ask** dashboard (2088105), each `environment = production` with internal users filtered, weekly over 90 days: time to first output p50 and p90 by `judge_mode` (`Jg7y5xmT`), total latency p50 and p90 by `judge_mode` (`lz4upVnP`), Jev failure rate (`ND2XYXUq`), Jev time for the turn judgment and the passage check (`Kj3iDQ3t`), passages kept per turn (`yuK9D9fK`), turns with no writing-model call (`i6EUIOFN`), fallback rate in mode `on` (`m628nAZg`), time to first output by writing model (`Di0p07wj`), and case studies asked about before they are written (`e17Qoiwi`, `handoff_reason = case_study` by `page_path`, empty until that code ships). The spec's failure-rate tile became two (a rate and a time do not share an axis), and the shadow agreement tile was not built: production skipped its shadow week, so it would never hold a point.
- **What they hold today.** One production turn in mode `on` (2026-09-19, first output 2,669 ms). Team browsers carry the `sas_internal` cookie and are never captured, so the tiles fill only from visitors: at about 80 turns a month, read them over weeks.
- **The internal cookie, checked.** Server capture has no consent gate, so every production turn from an unmarked browser yields an `ask_questioned` event. Of the four turns in production's `ask-questions` on 2026-09-21, three have no event (2026-09-19 20:10 and 21:30, 2026-09-21 14:54) and one has (2026-09-19 22:27, a browser without the cookie). A stored turn with no event is a team browser: that comparison is the way to verify `?internal=on` took, per browser.

### Still owed before the judge decides anything in production

The ordered list, with commands and done-when checks, is [Next steps](#next-steps) at the top of this document. In short: the key in Vercel, `on` for preview, TypeSafe on the privacy page, production `shadow` and then `on` (all done by 2026-09-21), and still owed: the human read of the answers and one pass of the manual script on the live site.
