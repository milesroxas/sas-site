...
- **Drafts & versions** — Website collections and Content Hub narrative collections use drafts with autosave, version history (max 50), and scheduled publishing. The jobs runner hits `/api/payload-jobs/run` every 10 minutes from a Vercel cron (`vercel.json`), with a best-effort GitHub Actions workflow (`.github/workflows/payload-jobs.yml`) as backstop; both authenticate with `CRON_SECRET`.
