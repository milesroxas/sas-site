...
The preview uses the isolated `streak_studio_preview` database on the existing Neon project, with branch-specific Vercel connection variables. The project-managed Neon account was not accessible to the local Neon CLI, so this is a separate database cloned from production, not a Neon branch. Production records and the production migration ledger are unchanged.
