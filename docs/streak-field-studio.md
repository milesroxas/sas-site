...
A look has one **draft** and a list of **releases**. The draft is always editable and autosaves. A release is immutable and numbered (v1, v2). Publish freezes the draft as the next release; Restore and Reset to release copy a release's settings back into the draft. Nothing ever edits a release, so a page that pinned v2 keeps v2.

1. **Studio tab.** The stage shows the live field at a placement (hero, block, menu, card) on a dark or light ground, with a budget line under it (particles, DPR, octaves, segments; amber when the placement cap bites). Starters load a shipped look into the draft. Releases lists the draft, a kept comparison and every release: clicking a release shows it on the stage beside the draft (looking never changes anything), and its Restore makes those settings the draft. The header says where the draft stands ("Draft matches v2", "Draft, 4 changes since v3", rendering or failed posters) and, when the draft matches no release, offers Reset to release, which puts the published release's settings back. Both are one undo step. Draft and Compare switch the stage between the draft and the comparison; any edit, undo or redo returns the stage to the draft. Seed and capture frame sit under the stage. Undo and redo hold fifty steps for the page (⌘Z, ⇧⌘Z). Space pauses, R randomizes the seed, C toggles the comparison while the stage has focus.
...
3. **Publish release** saves the draft and queues exact-frame light and dark posters. Close the page if you like; the Renders tab shows progress, errors, cancellation and retry, and the Releases tab lists every immutable release with its posters, Restore as draft and Compare on stage. Publishing settings a release already has renders nothing: the look is pointed at that release.
...
Publishing does not publish a page. When the posters land, the look itself is published at the new release: its published document carries the release's recipe and poster, which is what the library thumbnail and Payload's own Status line (Changed, Revert to published) read. If someone edited the look while its posters rendered, the release is still published and the newer draft goes back on top, so the document reads Changed and no work is lost. Republishing identical settings reuses the completed release. Restoring and publishing older settings does not rewrite earlier releases.

A release is its snapshot. `recipeHash` (`plugins/streak-studio/hash.ts`) hashes the resolved snapshot in canonical, key-sorted form, because Postgres `jsonb` returns keys reordered and the editor keeps the order it built; "draft matches release" in the stage uses the same canonical comparison. Recipes that resolve to the same snapshot are the same release, so a restored release republishes as itself. A look can only be published at a release: the `beforeChange` guard accepts output that has a release, or output that is already published (Revert to published), and `promoteRelease` (`plugins/streak-studio/releases.ts`) is the one write that carries a pass. Releases published before 2026-09-18 hold hashes from the earlier recipe-based scheme: they keep working everywhere, and republishing their exact settings once creates a new release instead of reusing them.
