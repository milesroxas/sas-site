...
MCP cannot carry a binary, so this is the one way an agent adds media. It uses the MCP API key the agent already has (`CMS_MCP_API_KEY`, else `SAS_CMS_MCP_KEY`: the same string the MCP client sends) and posts to `POST /api/agent/media`. Target site: `CMS_UPLOAD_SERVER`, else `NEXT_PUBLIC_SERVER_URL`. Prints the media id.

The target has to be the site the agent's MCP server drafts on, because a media id only exists in one database. The configured `sas-cms` server is production while `NEXT_PUBLIC_SERVER_URL` in a workspace is its dev server, so [`scripts/cms-target.ts`](../scripts/cms-target.ts) refuses a local site unless `--local` is passed (right only when the MCP client points at that dev server too). How an agent takes the screenshot in the first place, through the person's own Chrome, is in the article-authoring skill.
