...
- Block spacing: every composition block's root is `Section` (or the work-page `RevealSection`), which owns the vertical rhythm via `spacing`: `normal` (default) / `tight` / `loose` (media-forward) / `none` (block owns a pinned shell). Blocks never write their own `py-*`/`my-*`, renderers never add margin, and theme never changes spacing. A block nested inside the editor-facing `section` block renders `bare`; the Section's `SectionBand` (`src/blocks/section/`) paints the band once and owns the internal stack gap. Full-viewport height (`fullViewportSectionClassName`) is for page-level sections only, never a composition block.
- Block horizontal layout: reorganized blocks place content on `BlockGrid` (`src/blocks/shared/grid.tsx`): 8 columns from `md`, one `--spacing-grid` gap token, children placed with `md:col-start-*`/`md:col-span-*` only. A cell that is itself a run of cells (a list beside a heading) is `<BlockGrid subgrid>`, never a nested grid. Never restate tracks/gap/breakpoint, no `max-w-*` on grid cells, no spacer tracks. Contract and migration map: [docs/block-grid-roadmap.md](docs/block-grid-roadmap.md).
...
| Shared agent skills + plugins (GSAP, R3F, pmndrs docs MCP, adding/updating skills) | [docs/agent-skills.md](docs/agent-skills.md) |
