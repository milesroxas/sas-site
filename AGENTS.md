...
- Block horizontal layout: reorganized blocks place content on `BlockGrid` (`src/blocks/shared/grid.tsx`): 8 columns from `md`, one `--spacing-grid` gap token, children placed with `md:col-start-*`/`md:col-span-*` only. Never restate tracks/gap/breakpoint, no `max-w-*` on grid cells, no spacer tracks. Contract and migration map: [docs/block-grid-roadmap.md](docs/block-grid-roadmap.md).
