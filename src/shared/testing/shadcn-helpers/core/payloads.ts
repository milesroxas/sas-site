...
export type ChatPayloads = {
  ...
  sourceDocument(overrides?: Partial<SourceDocumentPayload>): SourceDocumentPayload
  custom(kind?: string, options?: { providerMetadata?: JsonRecord }): CustomPayload
}
...
export function createMetadataFactory<METADATA>(now?: Date | string) {
  const resolvedNow = now instanceof Date ? now : new Date(now ?? '2026-01-01T00:00:00.000Z')
