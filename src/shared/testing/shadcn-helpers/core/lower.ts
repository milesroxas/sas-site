import type { ChatEvent, DataTypes, NeutralChunk, StreamStep, ToolSet } from './types'
...
function lowerTextEvent<METADATA, DATA extends DataTypes, TOOLS extends ToolSet>(
  ...
  const deltas = event.options?.mode === 'instant' ? [event.text] : splitTextDeltas(event.text)
 ...
 */
export function lowerEvents<METADATA, DATA extends DataTypes, TOOLS extends ToolSet>(
