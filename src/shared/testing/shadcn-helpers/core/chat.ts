...
>(
  ...
  function findLatestChatIndex(messages: readonly MESSAGE[]) {
    const messageIds = new Set(messages.map((message) => format.getMessageId(message)))
    ...
    if (latestChatIndex === -1) {
      ...
      latestChatIndex = turns.findIndex(
        (turn) =>
          ...
          format.getMessageRole(turn.message) === format.getMessageRole(lastMessage) &&
          format.getMessageText(turn.message) === format.getMessageText(lastMessage),
      )
    }
  ...
  function findNextAssistantTurn(messages: MESSAGE[], messageId?: string) {
    if (messageId) {
      const index = turns.findIndex((turn) => format.getMessageId(turn.message) === messageId)
    ...
    return turns.slice(latestChatIndex + 1).find((turn) => turn.role === 'assistant')
  }
  ...
  const api: Chat<MESSAGE, PART, TRANSPORT, METADATA, DATA, TOOLS, WRITER> = {
    user(text = DEFAULT_TEXT, userOptions: ChatUserOptions<METADATA> = {}) {
      ...
      const turnMetadata =
        userOptions.metadata === undefined ? metadata() : cloneValue(userOptions.metadata)
    ...
    assistant(
      input: string | PART[] | ((context: { writer: WRITER }) => void) = DEFAULT_TEXT,
      assistantOptions: ChatAssistantOptions<METADATA> = {},
    ) {
      const events = takePendingEvents()
      const messageParts = materializeAssistantInput(input, events)
      const turnMetadata =
        assistantOptions.metadata === undefined ? metadata() : cloneValue(assistantOptions.metadata)
    ...
    transport(transportOptions: ChatTransportOptions<MESSAGE, PART, DATA, TOOLS, WRITER> = {}) {
