...
import type { Post, SiteInfo } from '@/payload-types'
...
/**
 * Shape both llms documents share: resolve the site context, render every
 * content section through `renderSection`, and join what came back under the
 * common header. `trailingParts` appends the blocks only one of them carries.
 */
const buildLlmsDocument = async (
  payload: Payload,
  renderSection: (section: AeoContentSection, siteUrl: string) => Promise<string | null>,
  trailingParts: (siteInfo: Partial<SiteInfo>) => (string | null)[] = () => [],
): Promise<string> => {
  const siteUrl = getServerSideURL()
  const siteInfo = await getSiteInfo(payload)

  const sections = await Promise.all(
    AEO_CONTENT_SECTIONS.map((section) => renderSection(section, siteUrl)),
  )

  const parts = [buildHeader(siteInfo, siteUrl), ...sections, ...trailingParts(siteInfo)]
 ...
 */
export const buildLlmsTxt = (payload: Payload): Promise<string> =>
  buildLlmsDocument(
    payload,
    async (section, siteUrl) => {
 ...
 */
export const buildLlmsFullTxt = (payload: Payload): Promise<string> =>
  buildLlmsDocument(payload, async (section, siteUrl) => {
...
/**
 * Front matter for a standalone post: title, the summary blockquote when the
 * post carries one, then the lines a model needs to cite and date the page.
 */
const buildPostHeader = (post: Post, siteUrl: string): string =>
  [
    `# ${post.title}`,
    '',
...
export const buildPostMarkdown = async (
  ...
  return `${buildPostHeader(post, siteUrl)}\n\n${body}\n`
}
