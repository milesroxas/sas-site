...
export const MCP_INSTRUCTIONS = [
  ...
  'Edit one block: locateBlock or outlineDocument finds it, getBlock and patchBlock take its id. A whole-document update must keep every block `id`: blocks sent without ids replace the rows.',
  'Relationship fields take document ids: look them up with a find tool.',
  'Omit `slug`, `key` and `generateSlug`; if a create tool requires one, send a short URL-safe value, never an email.',
  ...
  'Add images with `pnpm cms:upload` (steps in the article-authoring skill).',
