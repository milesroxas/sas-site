  ...
  "group/alert relative grid w-full gap-0.5 rounded-lg border px-2 py-1.5 text-left text-xs/relaxed has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-1.5 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        ...
        success:
          'border-success bg-success/30 text-card-foreground *:data-[slot=alert-description]:text-card-foreground/80',
        warning:
          'border-warning bg-warning/30 text-card-foreground *:data-[slot=alert-description]:text-card-foreground/80',
        error:
          'border-error bg-error/30 text-card-foreground *:data-[slot=alert-description]:text-card-foreground/80',
      },
    },
    defaultVariants: {
      variant: 'default',
