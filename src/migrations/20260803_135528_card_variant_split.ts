...
/**
 * Adds archive `cardVariant` enum value `split` and makes it the column default.
 *
 * Do NOT use `ALTER TYPE ... ADD VALUE` then `SET DEFAULT` in the same migration:
 * Payload wraps each migration in a transaction, and Postgres forbids using a
 * newly added enum value until that transaction commits ("unsafe use of new value").
 * Recreate the enum types instead so ADD + DEFAULT stay in one transactional migration.
 */
