...
type AudienceTab = RowWithVisual<AudienceTabsBlockData['tabs'][number]>

type AudienceTabsProps = {
  heading: AudienceTabsBlockData['heading']
  tabs: AudienceTab[]
  theme?: AudienceTabsBlockData['theme']
}
...
export const AudienceTabs: React.FC<AudienceTabsProps> = ({ heading, tabs, theme }) => {
  const panels = tabs ?? []
  ...
  // Outgoing visual kept mounted under the incoming one while a swap wipes in.
  const [prevVisual, setPrevVisual] = useState<VisualValue | null>(null)
  ...
  const selectTab = useAudienceTabsMotion({
    ...
    textIndex,
    onSelect: (index) => {
      setPrevVisual(panels[active]?.visual ?? null)
      setActive(index)
    },
    onTextSwap: setTextIndex,
    onSettled: () => setPrevVisual(null),
  })
    ...
    <Section spacing="loose" theme={bandTheme}>
      ...
      >
        ...
        <div className="grid w-full grid-cols-1 items-stretch gap-8 md:gap-12 lg:grid-cols-12  lg:gap-x-12">
          ...
          >
            {prevVisual ? (
              // The underlay only backs the wipe: placement `card` keeps a
              // Streak Field on its poster there, so the one live slot stays
              // with the incoming tab.
            ...
            <div className="absolute inset-0" data-swap="media">
              {visual ? (
                <Visual
                  ...
                  htmlElement={null}
                  imgClassName="object-cover"
                  // Fresh element per tab so the swap wipes a new node in
                  // over the underlay instead of mutating the old one.
