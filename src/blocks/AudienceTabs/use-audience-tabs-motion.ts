 ...
 * because the entrance is play-once: it finishes before any swap can start
 * (targets are hidden until it plays), and never touches the nodes again.
 */
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        if (media) {
          // Media closes the sequence: it starts a beat after the last list
          // item begins, however many items the panel has.
          const mediaAt =
            AUDIENCE_TABS_MOTION.itemsAt +
            Math.max(items.length - 1, 0) * AUDIENCE_TABS_MOTION.itemStagger +
