...
That is the flow with the judge `off`: the code's default, and what any environment without `TYPESAFE_API_KEY` and `ASK_JEV` runs. Since 2026-09-21 production, preview and local dev all run `ASK_JEV=on`, so the flow visitors meet is the one below. With `ASK_JEV=on` the decisions move out of the writing model:
...
| [`judge.ts`](./judge.ts) | The judge, server only: `askJudgeMode()` (`ASK_JEV`, always `off` without `TYPESAFE_API_KEY`), a lazy TypeSafe client pinned to one Jev version with an 800 ms timeout and no retries, `judgeTurn()` and `judgePassages()` (both answer null on any failure and never throw), every threshold in `ASK_JUDGE_THRESHOLDS`, and the pure routing: `routeTurn()`, `routePassage()`, `dependsOnPrevious()`. The question is redacted before it is sent. Tested in `judge.test.ts` (routing, mode parsing, null handling; no network). |
...
| [`storyBrief.ts`](./storyBrief.ts) | Server only: what the site can honestly say about a case study whose story is not written yet. `resolveStoryBrief()` reads the work page's case study and project as the public does (two reads per page per instance per minute) into a brief: client, kinds of work (featured capabilities first), industries, platforms, the fullest summary, deliverables. `storyWords()` counts the six story sections and `THIN_STORY_WORDS` (200) says when the story is thin. `namesStory()` is code's lookup for a question that names the page or the client; `withStoryBrief()` puts the brief first among the sources. Tested in `storyBrief.test.ts`. |
...
| [`prompts.ts`](./prompts.ts) | `askSystemPrompt()`: the system prompt per turn, grounded or chat-only, with the handoff tool's rules while the tool is on offer and without a word of it once the visitor has sent (`offersAskHandoff`), so the prompt and the tool list never disagree. Every turn without the tool (a routed turn, or after the visitor has sent) is told this chat cannot send anything to the team and warmly guides a visitor who asks to the "Talk to the team" form (a next step, never a limit), so a reply never claims a handoff that did not happen. Tested in `prompts.test.ts`. |
...
| `ASK_JEV` | The judge's mode: `off` (the default when unset; the writing model decides, with the `handoff` tool), `shadow` (Jev runs beside that path and is measured, deciding nothing), `on` (Jev routes the turn and checks passages). Set to `on` in production, preview and local dev since 2026-09-21. Rollback is this value and a redeploy, no code revert. |
...
- **One more processor with the judge on.** In `shadow` and `on` the redacted question (and, for the passage check, the site's own published passages) goes to TypeSafe. It does not train on requests; zero retention is an enterprise plan. The published Privacy Policy lists it (sections 6 and 7), and must keep doing so while the judge runs.
...
A work page can publish with little more than its images. The record behind it then holds a client, the kinds of work, a summary and maybe deliverables, and "What did you do for them?" used to get a vague answer from whatever chunk ranked, or "the site doesn't cover that". With `ASK_JEV=on`:

- **Thin is arithmetic, in code.** `storyWords()` counts the words in the case study's six story sections (overviews and beats). Under `THIN_STORY_WORDS` (200) the story is thin. Measured on production 2026-09-21: written case studies hold 411 to 732 words across four to six sections (Interchecks 444), unwritten ones 82 to 148 in one or two (GentleBeast, Arturo, eClinical Solutions). What the page's layout chooses to render is not the measure: Ask answers from the record, so a page of images over a written record is not thin.
- **The turn has to route first.** "What did you do for Arturo?" read to Jev as no request at all (`request` confidence 0.15 to 0.22, so every such turn fell back to the judge-off path) until the `information` criterion named past work for a client and `project` ruled it out. After: 1.00, with the turn fixture at 21 of 21 and the journey fixture at 13 of 13 (2026-09-21). Two cases in `ASK_CASES` (`past-work-named`, `past-work-project`) hold it.
- **Whether the question is about the page is the judge's and code's.** The page is the subject when the question leaves its subject to it (Jev's `open_reference`, the same Noul that attaches the page search) or names the page or client (`namesStory()`, an exact lookup, so it stays in code). A follow-up that leans on the turn before it (`depends_on_previous`) is about that turn's subject, which may be another client, unless it names this one. No new Jev question was needed.
- **The answer.** The brief leads the sources (`withStoryBrief()`), merged with whatever retrieval kept of the same page, so the kinds of work are always there to name and a passage check that kept nothing no longer ends in `no_answer`. One prompt line tells the writing model to answer from the outline, naming the kinds of work, and to leave "unfinished" unsaid.
- **The card.** Code closes the reply with `case_study`: "The full case study is still being written. A partner can walk you through it and similar work." Said by the card, not the model, so it is said every time and worded once. The turn's own reason wins when it has one: pricing their own project on a thin case study is still `estimate`. Once the visitor has sent there is no card, and the prompt asks the reply to say the story is still being written instead.
- **What it costs.** Two small cached reads, started beside the Jev request and the embedding, so nothing is added to the path to first output. `story_thin` on `ask_questioned` counts the turns, and `handoff_reason = case_study` with `page_path` says which case study to write next.

Production, preview and local dev run `on`, so this is live. In `off` and `shadow` (the rollback) none of it runs, like the rest of the journey: without the judge nothing says the question is about the page, and the writing model cannot be trusted to write words and call a tool in one turn.
